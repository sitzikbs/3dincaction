...
def get_graph_feature(x, k=20, idx=None):
    ...
    if idx is None:
        idx = knn(x, k=k)  # (b * t, num_points, k)

    idx_base = torch.arange(0, b * t, device=x.device).view(-1, 1, 1) * n
    idx = idx + idx_base
    idx = idx.view(-1)

    x = x.transpose(2, 1).contiguous()  # (b, n, d)  -> (b*n, d) #   b * n * k + range(0, b*n)
...
class DGCNN(nn.Module):
    def __init__(self, model_cfg, num_class=14, k=20, emb_dims=1024, dropout=0.5, n_frames=32):
