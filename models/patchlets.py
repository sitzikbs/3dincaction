...
class PatchletsExtractor(nn.Module):
    ...
    def forward(self, point_seq, feat_seq=None):
        ...
        # patchlet_feats = torch.empty(b*t, n, self.k, d_feat, device=feat_seq.device)
        ...
        patchlets[0] = idxs[0]
        # patchlet_points[0] = x1[0, patchlets[0]]
        # patchlet_feats[0] = feat_seq[0, patchlets[0]]
        ...
        for i in range(1, len(x1)):
            ...
            # patchlet_points[i] = xb[patchlets[i]]
            # patchlet_feats[i] = feat_seq[i][patchlets[i]]
...
class PointNet2PatchletsSA(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if points is None:
            patchlet_dict = self.patchlet_extractor(new_xyz, None)
        else:
            patchlet_dict = self.patchlet_extractor(new_xyz, new_points)
        ...
        return patchlet_dict['patchlet_points'][:, :, :, 0, :].permute(0, 1, 3, 2), patchlet_features.permute(0, 1, 3, 2)
...
class PointNet2Patchlets(nn.Module):
    def __init__(self, num_class, n_frames=32, in_channel=3):
        ...
        self.sa1 = PointNet2PatchletsSA(npoint=512, radius=0.2, nsample=32, in_channel=in_channel,
                                              mlp=[64, 64, 128], group_all=False, k=16, temporal_conv=8)
