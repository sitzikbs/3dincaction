...
class FourDmFVNet(torch.nn.Module):
  def __init__(self, model_cfg, num_class, n_frames=16, normalize=True):
    """
    ...
    self.do2 = torch.nn.Dropout(0.3)
    self.fc3 = torch.nn.Linear(256, 128)
    self.bn3 = torch.nn.BatchNorm1d(128)
    self.do3 = torch.nn.Dropout(0.3)
    self.fc4 = torch.nn.Linear(128, num_class)


  def forward(self, points):
      points = points.permute([0, 1, 3, 2])
      batch_size, n_frames = points.shape[0], points.shape[1]
      points = points.reshape(-1, points.shape[2], points.shape[3])
      with torch.no_grad():
      ...
      global_featuer = self.globalfeature(ThreeDmFV.view(batch_size, n_frames, 20, self.n_gaussians, self.n_gaussians,  self.n_gaussians))
      global_featuer = global_featuer.view(batch_size*n_frames, -1)
      x = F.relu(self.bn1(self.fc1(global_featuer)))
      x = self.do1(x)
      ...
      return {'pred': x.reshape(batch_size, n_frames, -1).permute(0, 2, 1), 'features': global_featuer}
...
class Global3DmFVFeature(torch.nn.Module):
    def __init__(self, n_gaussians):
        super(Global3DmFVFeature, self).__init__()
        """
...
class Global4DmFVFeature(torch.nn.Module):
    def __init__(self, n_gaussians):
        super(Global4DmFVFeature, self).__init__()
        """
        architecture for learning the global feature from 3DmFV representation. It changes depending on the numbre of
        Gaussians
        :param n_gaussians: number of gaussians in 3DmFV representation
    ...
    def forward(self, x):
        batch_size, n_frames, fvd, _, _, _ = x.shape
        x = x.view(batch_size*n_frames, fvd, self.n_gaussians, self.n_gaussians, self.n_gaussians)
        x = self.inception1(x)
        x = self.inception2(x)
        x = self.inception3(x)
        x = self.maxpool1(x) if self.maxpool1 is not None else x
...
if __name__ == '__main__':
  ...
  n_frames, batch_size, K, n_points = 16, 64, 8, 128

  # Create random Tensors to hold inputs and outputs
  points = torch.randn(n_frames, batch_size, n_points, 3, dtype=torch.float32).cuda()
  y = torch.randn(batch_size*n_frames, 40).cuda()

  # Construct our model by instantiating the class defined above.
  model = FourDmFVNet(K, 40, normalize=True).cuda()
