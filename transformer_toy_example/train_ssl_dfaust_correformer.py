...
# Set up data
train_dataset = NoiseGenerator(args.n_points, radius=0.5, n_samples=131072, sigma=0.3)
...
test_dataset = Dataset(args.dataset_path, frames_per_clip=args.frames_per_clip + 1, set='test', n_points=args.n_points,
                       shuffle_points='each', gender='male')
        ...
        if eval_steps == args.eval_steps:
            ...
            with torch.no_grad():
                ...
                test_l1_mask = torch.max(test_gt_corr,
                                    1.0 * (torch.rand(args.batch_size, args.n_points, args.n_points).cuda() < test_gt_corr.mean()))
                ...
                test_loss_log_dict = {"acc": test_avg_acc, "losses/l1_loss": test_l1_loss.detach().cpu().numpy(),
                                 ...
                                 "idx_error": torch.abs(test_point_ids - test_max_ind).float().mean().detach().cpu().numpy()}
                log_scalars(test_writer, test_loss_log_dict, iter)
                model.train()

    if epoch % 5 == 0:
        # log images and save model
