...
class IKEAActionVideoClipMultiModalDataset(IKEAActionDataset):
    def __init__(self, dataset_path, db_filename='ikea_annotation_db_full', action_list_filename='atomic_action_list.txt',
                 action_object_relation_filename='action_object_relation_list.txt', train_filename='train_cross_env.txt',
                 test_filename='test_cross_env.txt', transform=None, set='test', camera='dev3', frame_skip=1,
                 frames_per_clip=64, resize=None, mode='vid', input_type='rgb', mod2='depth', n_points=1024):
    ...
    #     # return  torch.zeros([self.frames_per_clip, 270, 480, 3], dtype=torch.float32)
    #     return np.zeros([self.frames_per_clip, 270, 480, 3], dtype=np.float32)

    def load_multimodal_frames(self, video_full_path, frame_ind):
        ...
        for counter, i in enumerate(frame_ind):
            ...
            rgb_img_filename = os.path.join(video_full_path, str(i).zfill(6) + '.jpg')
            rgb_img = cv2.imread(rgb_img_filename)
            rgb_img = cv2.cvtColor(rgb_img, cv2.COLOR_BGR2RGB)
            rgb_frames.append(rgb_img)

            if self.mod2 == 'depth':
                depth_img_filename = os.path.join(video_full_path.replace('images', 'depth'), str(i).zfill(6) + '.png')
                depth_img = cv2.imread(depth_img_filename)
                depth_img = cv2.cvtColor(depth_img, cv2.COLOR_BGR2RGB)
                mod2_frames.append(depth_img)
            else:
                pc_filename = os.path.join(video_full_path.replace('images', 'point_clouds'), str(i).zfill(6) + '.ply')
                ...
                if self.n_points is not None:
                    indices = torch.randperm(len(pc))[:self.n_points]
                    pc = pc[indices, :]
                mod2_frames.append(pc.t())
        if self.mod2 == 'depth':
            mod2_output = torch.from_numpy(np.asarray(mod2_frames, dtype=np.float32))
        else:
            mod2_output = torch.stack(mod2_frames, 0)
        return torch.from_numpy(np.asarray(rgb_frames, dtype=np.float32)), mod2_output
    ...
    def __getitem__(self, index):
        ...
        rgb_imgs, mod2 = self.load_multimodal_frames(video_full_path, frame_ind)
        rgb_imgs = torch.permute(rgb_imgs, [3, 0, 1, 2])  # permujte for transform of shape [..., H, W]
        rgb_imgs = self.transform(rgb_imgs)
        if self.input_type == 'depth':
            mod2 = torch.permute(mod2, [3, 0, 1, 2])  # permujte for transform of shape [..., H, W]
            mod2 = self.transform(mod2)

        return rgb_imgs, mod2, torch.from_numpy(labels), vid_idx, frame_pad
...
class IKEAPoseActionVideoClipDataset(IKEAActionVideoClipMultiModalDataset):
    def __init__(self, dataset_path, db_filename='ikea_annotation_db_full', action_list_filename='atomic_action_list.txt',
                 action_object_relation_filename='action_object_relation_list.txt', train_filename='train_cross_env.txt',
...
class IKEACombinedDataset(IKEAPoseActionVideoClipDataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            poses = self.load_poses(video_full_path, frame_ind)
            imgs = self.load_multimodal_frames(video_full_path, frame_ind)
